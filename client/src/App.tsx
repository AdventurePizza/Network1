...
function App() {
  ...
  const [profile, setProfile] = useState({ color: color, timestamp: Date.now(), key: tempID, username: "" });
  ...
  const handleKeyPress = (event) => {
    ...
    function updateStatus() {
      if (activeAccount) {
        ...
        setProfile({ ...profile, color: color, key: activeAccount.address, username: usernameInput, timestamp: timestamp })
        //add socket 
        socket.emit('profile', { ...profile, color: color, key: activeAccount.address, username: usernameInput, timestamp: timestamp });
        setProfileFB({ ...profile, color: color, key: activeAccount.address, username: usernameInput, timestamp: timestamp });
        setStatusHistory((statusHistory.concat([{ ...profile, color: color, key: activeAccount.address, username: usernameInput, timestamp: timestamp }])).sort((a, b) => parseFloat(a.timestamp) - parseFloat(b.timestamp)));
    ...
    <div >
      ...
      <div style={{ display: "flex", width: "90%", marginLeft: "auto", marginRight: "auto", overflowX: "scroll" }} >
        {statusHistory && statusHistory.map((profile) => (
          <div key={profile.timestamp} style={{ textAlign: "center", margin: 6 }}>
            <div style={{ width: 80, height: 20, backgroundColor: profile.color, border: "solid 4px ", marginInline: 4 }}>
            </div>
      ...
      <div style={{ width: "90%", marginLeft: "auto", marginRight: "auto" }} >
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <TextField id="outlined-basic" label="info" variant="outlined" placeholder="Status" size="small" onChange={handleChangeUsername} value={usernameInput} onKeyPress={handleKeyPress} />
